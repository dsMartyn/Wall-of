...
*/	
	...
	public function getMembers_paged($startIndex, $numItems) 
	{
	    ...
	    while (mysqli_stmt_fetch($stmt)) {
	      $rows[] = $row;
	      $row = new stdClass();
	      mysqli_stmt_bind_result($stmt, $row->RowID, $row->UserName,  $row->Password,  $row->FirstName,  $row->LastName,  $row->DOB,  $row->ContactNumber, $row->StartDates);
	    }
	...
	{
		$stmt = mysqli_prepare($this->connection, "INSERT INTO $this->memberTable (UserName, Password, FirstName, LastName, DOB, ContactNumber, StartDate) VALUES (?, ?, ?, ?, ?, ?, Now())");		
		$this->throwExceptionOnError();
		
		mysqli_bind_param($stmt, 'ssssss',$item->UserName,$item->Password,$item->FirstName,$item->LastName,$item->DOB,$item->ContactNumber);		
		$this->throwExceptionOnError();
	...
	/************** email_update_info($to - email address, $name - person) ***************\
		...
		returns void
	\*****************************/
	private function email_update_info($to, $name)
	{
	...
	public function updateMember($item) 
	{
		...
		$stmt = mysqli_prepare($this->connection, "UPDATE $this->memberTable SET UserName=?, Password=?, FirstName=?, LastName=?, DOB=?, ContactNumber=?	WHERE RowID=?");		
		$this->throwExceptionOnError();		
		
		mysqli_bind_param($stmt, 'ssssssi',$item->UserName,  $item->Password,  $item->FirstName,  $item->LastName,  $item->DOB,  $item->ContactNumber,$item->RowID);			
		$this->throwExceptionOnError();
	...
	{
		$stmt = mysqli_prepare($this->connection, "SELECT RowID FROM $this->memberTable where UserName=?");
		$this->throwExceptionOnError();
